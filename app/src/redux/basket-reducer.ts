...
import { includeInArray, increaseDecreaseCount, isChangeCount } from '../utils/helpWithBasket';
...
const basketReducer = (state = initialState, action: ActionsType): InintalStateType => {
    switch (action.type) {
        case SET_BASKET:
            return {
                ...state,
...
export const setBasket = (): ThunkAction<Promise<void>, any, unknown, any> => async (dispatch) => {
...
export const addToBasket = (product: ProductType, countProduct: number): ThunkAction<Promise<void>, any, unknown, any> => async (dispatch) => {
    let basketLS = localStorage.getItem('basket');
    if (basketLS) {
...
export const deleteFromBasket = (barcode: number): ThunkAction<Promise<void>, any, unknown, any> => async (dispatch) => {
    let basketLS = localStorage.getItem('basket');
    if (basketLS) {
        let basket: Array<ProductBasketType> = JSON.parse(basketLS);
...
export const increaseCountProduct = (barcode: number): ThunkAction<Promise<void>, any, unknown, any> => async (dispatch) => {
    let basketLS = localStorage.getItem('basket');
    if (basketLS) {
        let basket: Array<ProductBasketType> = JSON.parse(basketLS);
...
export const decreaseCountProduct = (barcode: number): ThunkAction<Promise<void>, any, unknown, any> => async (dispatch) => {
    let basketLS = localStorage.getItem('basket');
    if (basketLS) {
        let basket: Array<ProductBasketType> = JSON.parse(basketLS);
...
export const deleteBasket = (): ThunkAction<Promise<void>, any, unknown, any> => async (dispatch) => {

...
const CatalogCard: React.FC<PropsType> = ({ sort, addToBasket, productsFiltered }) => {
    const navigate = useNavigate();

    const productsSorted = useMemo(() => {
        return productsFiltered ? sortArray(productsFiltered, null, sort) : null
    }, [productsFiltered, sort]);

    const productcard = useMemo(() => {
        return productsSorted ? productsSorted.map((product: ProductType) => {
            return (
                <div key={product.barcode} className={style.card}
                    ...
                    <div className={style.card_buy}>
                        ...
                        <UIButton style={{ 'padding': '15px 25px' }} onClick={(e: React.SyntheticEvent<EventTarget>) => {
                            e.stopPropagation()
    ...
    }, [productsSorted, addToBasket, navigate])
    return (
